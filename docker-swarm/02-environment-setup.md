...
커널 모듈을 이해하려면 먼저 **일반 프로그램과 무엇이 다른지** 알아야 한다.

#### 일반 프로그램 vs 커널 모듈

```
일반 프로그램 (User Space):              커널 모듈 (Kernel Space):
┌─────────────────────────┐             ┌─────────────────────────┐
│  nginx, java, node 등    │             │  br_netfilter, ip_vs 등  │
│                         │             │                         │
│  • 사용자 공간에서 실행    │             │  • 커널 공간에서 실행      │
│  • OS 위에서 돌아감       │             │  • OS 자체의 일부가 됨     │
│  • 충돌해도 그 프로그램만  │             │  • 충돌하면 시스템 전체    │
│    죽음 (프로세스 격리)    │             │    패닉 가능 (보호 없음)   │
│  • 하드웨어 직접 접근 불가 │             │  • 하드웨어 직접 접근 가능  │
│  • apt/dpkg로 설치        │             │  • modprobe로 로드         │
└─────────────────────────┘             └─────────────────────────┘
         │                                       │
         ▼                                       ▼
┌──────────────────────────────────────────────────────────┐
...
리눅스 OS = 자동차
  ...
  일반 프로그램 = 승객
    • 차 안에 타고 있을 뿐, 엔진을 직접 만지지 못함
    • 내리면(종료하면) 차는 그대로 달림
    • 승객이 기절해도(크래시) 차는 멀쩡함

  커널 모듈 = 엔진에 끼우는 부품 (터보차저, ABS 모듈 등)
    • 엔진(커널)의 일부가 됨 — 같은 권한으로 동작
    • 잘못된 부품을 끼우면 엔진이 멈출 수 있음 (커널 패닉)
    • 필요할 때 끼우고(modprobe) 빼는(modprobe -r) 게 가능
```

| 항목 | 일반 프로그램 | 커널 모듈 |
|------|-------------|-----------|
| **실행 공간** | User Space (사용자 공간) | Kernel Space (커널 공간) |
| **권한** | 제한됨 (시스템 콜로만 커널에 요청) | **무제한** (메모리, 하드웨어 직접 접근) |
| **설치 방식** | `apt install nginx` | `modprobe br_netfilter` |
| **파일 형식** | ELF 바이너리 (`/usr/bin/nginx`) | `.ko` 커널 오브젝트 (`/lib/modules/.../ip_vs.ko`) |
| **실행 확인** | `ps`로 보임 (PID가 있음) | `ps`에 안 보임, `lsmod`로 확인 |
| **충돌 시** | 해당 프로세스만 죽음 | **시스템 전체 패닉 가능** |
| **메모리 보호** | 있음 (가상 메모리로 격리) | 없음 (커널과 같은 메모리 공간 공유) |

#### 왜 모듈 구조인가?

리눅스 커널은 모놀리식(monolithic)이지만, 모든 기능을 한꺼번에 메모리에 올리지는 않는다. 필요한 기능만 **모듈(module)**로 분리해서 필요할 때만 로드한다.
...
모듈로 분리하면:
  ...
  ┌─ 커널 (코어만) ────────────┐
  │  프로세스 관리, 메모리 관리  │  ← 항상 로드 (작음)
  │  기본 네트워크, 기본 FS     │
  └───────────┬────────────────┘
              │ 필요할 때만 modprobe
              ▼
  ┌─ 모듈들 ─────────────────────────────────┐
  │  br_netfilter  (Docker가 필요할 때)       │
  │  vxlan         (Swarm이 필요할 때)        │
  │  ip_vs         (로드밸런싱 필요할 때)      │
  │  nvidia.ko     (GPU 쓸 때)               │
  │  ...                                     │
...
#### Docker Swarm이 커널 모듈을 필요로 하는 이유

Docker Swarm의 VXLAN 캡슐화나 IPVS 로드밸런싱은 **네트워크 패킷을 커널 레벨에서 직접 조작**해야 한다. 일반 프로그램(User Space)에서는 이런 저수준 네트워크 조작이 불가능하기 때문에, 반드시 커널 모듈이 로드되어 있어야 한다.
